...
def build_rag_evidence_pack(
    *,
    ...
    lower_context = compact_context.lower()
    ...
    scenario = (
        f"문서 검색 결과를 바탕으로 '{topic}'와 관련된 실무 상황을 판단하려고 합니다."
    )

    correct_points = [
        "문서에서 제시한 핵심 개념과 검증 기준을 근거로 판단합니다."
    ]

    wrong_points = [
        "문서 근거 없이 일반적인 LLM 응답 품질 조치만 선택합니다.",
        "프롬프트 지시만으로 검증이 자동 보장된다고 판단합니다.",
        "서버 측 검증 없이 결과를 그대로 실행합니다.",
        "필수 필드나 범위 오류를 저장 단계에서 허용합니다.",
    ]
    ...
    if any(k in lower_context for k in ["structured output", "json schema", "required", "enum", "answer_range", "validation_error"]):
        concepts = [
            "structured output",
            "JSON schema",
            "required field",
        ...
        scenario = (
            "AI 문제 생성 API에서 LLM이 JSON 형식의 응답을 반환했습니다. "
            "choices는 5개지만 answer 값이 6으로 들어와 저장 단계에서 검증 오류가 발생했습니다. "
            "문서에서는 required 필드, enum 값, answer 범위 같은 schema 및 비즈니스 규칙 검증이 필요하다고 설명합니다."
        )

        correct_points = [
            "answer 값이 1~5 범위를 벗어나면 저장하지 않고 retry 또는 repair 대상으로 처리합니다."
        ]

        wrong_points = [
            "JSON 형태로 응답했다는 점만 보고 저장 단계로 넘깁니다.",
            "프롬프트에 JSON 작성을 요청했으므로 별도 schema 검증을 낮은 우선순위로 둡니다.",
            "LLM이 의도한 의미가 맞다고 보고 enum 오류를 저장 단계에서 허용합니다.",
            "choices 개수만 맞으면 answer 범위 오류를 별도 실패 조건으로 보지 않습니다.",
        ]

        log_or_metric = {
            "response_format": "json_schema",
            "required": ["title", "body", "choices", "answer", "explanation"],
            "choices_count": 5,
            "answer": 6,
            "answer_range": "1..5",
            "validation_error": "answer is out of range",
            "action": "reject_and_retry_or_repair",
            "rag_context_summary": compact_context,
        }

    elif any(k in lower_context for k in ["tool calling", "function calling", "tool arguments", "function schema", "tool name"]):
        concepts = [
            "tool calling",
            "function calling",
            "tool arguments",
        ...
        scenario = (
            ...
            "문서에서는 tool arguments에 허용되지 않은 필드가 포함될 수 있으므로, "
            "실행 전에 서버 측 schema와 비즈니스 규칙 검증이 필요하다고 설명합니다."
        )

        correct_points = [
            "tool name과 arguments를 실행 전에 서버 측 schema와 허용 규칙으로 검증합니다."
        ]

        wrong_points = [
            "LLM이 반환한 tool arguments를 검증 없이 바로 실행합니다.",
            "tool calling을 사용하면 prompt injection으로 인한 인자 조작은 발생하지 않는다고 판단합니다.",
            "함수 이름만 맞으면 arguments의 타입과 허용 필드는 확인하지 않습니다.",
            "도구 호출 실패 시 사용자에게 그대로 오류를 노출합니다.",
        ]
        ...
        log_or_metric = {
            ...
            "tool_arguments": {
                ...
                "choices": ["A", "B", "C", "D", "E"],
                "answer": 6,
                "unexpected_field": "raw_sql",
            },
            "validation_error": "unexpected field and answer out of range",
            "action": "validate_before_execution",
            "rag_context_summary": compact_context,
        }

    elif any(k in lower_context for k in ["hybrid search", "rrf", "vector search", "keyword search", "fulltext"]):
        concepts = [
            "hybrid search",
            "vector search",
            "keyword search",
        ...
        scenario = (
            ...
            "문서에서는 의미 유사도와 정확한 용어 일치를 함께 활용하기 위해 hybrid search와 RRF 기반 병합을 사용할 수 있다고 설명합니다."
        )
        ...
        wrong_points = [
            "vector similarity만 높으면 항상 가장 좋은 context라고 판단합니다.",
            "keyword search 결과는 의미 검색이 아니므로 항상 제외합니다.",
            "vector score와 keyword raw score를 단순 합산합니다.",
            "top_k만 늘려 검색 방식 차이를 해결합니다.",
        ]

        log_or_metric = {
            "vector_result": "의미적으로 가까운 일반 설명 chunk",
            "keyword_result": "정확한 용어가 포함된 chunk",
            "fusion_method": "RRF",
            "issue": "vector 검색과 keyword 검색의 상위 결과가 다름",
            "rag_context_summary": compact_context,
        }
