...
def build_ai_advanced_v2_questions(
    *,
    topic: str,
    count: int,
    ...
    for _ in range(count):
        template_question = build_ai_advanced_template(
            topic=topic,
            exclude_formats=used_formats,
        )

        template_question = _normalize_advanced_template_question_text(template_question)

        template_format = template_question.get("template_format")
        if template_format:
            used_formats.append(template_format)

        questions.append(
            GeneratedQuestion(
                title=template_question["title"],
                body=template_question["body"],
                choices=template_question["choices"],
                answer=template_question["answer"],
                explanation=template_question["explanation"],
                difficulty="고급",
                competency_type="ai",
                competency_tags=template_question.get("competency_tags", ["AI"]),
                score=template_question.get("score", 5),
                question_format=template_question.get("template_format"),
                answer_style="best_action",
                evidence_summary={
                    "generation_source": "advanced_template",
                    "template_format": template_question.get("template_format"),
                    "answer_intent": template_question.get("answer_intent"),
                    "distractor_intents": template_question.get("distractor_intents", []),
                    "lock_choices": template_question.get("lock_choices", True),
                },
            )
        )
...
def _to_polite_sentence(text: str) -> str:
    """
    AI 고급 template 문체 정규화.
    템플릿 원문은 '-다' 문어체로 작성해도 되고,
    최종 저장 전 여기서 존댓말 종결로 일괄 변환한다.
    """
    ...
    value = value.replace("없 다.", "없다.")
    value = value.replace("있 다.", "있다.")
    value = value.replace("한 다.", "한다.")
    value = value.replace("된 다.", "된다.")
    value = value.replace("본 다.", "본다.")
    value = value.replace("미룬 다.", "미룬다.")
    value = value.replace("없앤 다.", "없앤다.")
    ...
    question_patterns = [
        ...
        (r"어떤 설계인가\?$", "어떤 설계입니까?"),
        (r"어떤 조치인가\?$", "어떤 조치입니까?"),
        (r"가장 적절한 방법은\?$", "가장 적절한 방법은 무엇입니까?"),
        (r"가장 적절한 판단은 무엇인가\?$", "가장 적절한 판단은 무엇입니까?"),
        (r"가장 적절한 접근은 무엇인가\?$", "가장 적절한 접근은 무엇입니까?"),
        (r"가장 적절한 설계는 무엇인가\?$", "가장 적절한 설계는 무엇입니까?"),
        (r"가장 적절한 조치는 무엇인가\?$", "가장 적절한 조치는 무엇입니까?"),
        (r"가장 우선적으로 점검해야 할 사항은 무엇인가\?$", "가장 우선적으로 점검해야 할 사항은 무엇입니까?"),
        (r"가장 우선적으로 검토해야 할 판단은 무엇인가\?$", "가장 우선적으로 검토해야 할 판단은 무엇입니까?"),
        (r"가장 우선적으로 추가해야 할 파이프라인 단계는 무엇인가\?$", "가장 우선적으로 추가해야 할 파이프라인 단계는 무엇입니까?"),
    ]
    ...
    ordered_replacements = [
        ...
        ("확인하지 않고 있다.", "확인하지 않고 있습니다."),
        ("선별하지 않고 있다.", "선별하지 않고 있습니다."),
        ("검증하지 않고 있다.", "검증하지 않고 있습니다."),
        ("구분하지 않고 있다.", "구분하지 않고 있습니다."),
    ...
    ability_patterns = [
        (r"줄일 수 있다\.", "줄일 수 있습니다."),
        (r"높일 수 있다\.", "높일 수 있습니다."),
        (r"개선할 수 있다\.", "개선할 수 있습니다."),
        (r"확인할 수 있다\.", "확인할 수 있습니다."),
        (r"판단할 수 있다\.", "판단할 수 있습니다."),
        (r"적용할 수 있다\.", "적용할 수 있습니다."),
        (r"비교할 수 있다\.", "비교할 수 있습니다."),
        (r"측정할 수 있다\.", "측정할 수 있습니다."),
        (r"검토할 수 있다\.", "검토할 수 있습니다."),
        (r"약해질 수 있다\.", "약해질 수 있습니다."),
        (r"늘어날 수 있다\.", "늘어날 수 있습니다."),
        (r"도움이 될 수 있다\.", "도움이 될 수 있습니다."),
    ]

    for pattern, replacement in ability_patterns:
        value = re.sub(pattern, replacement, value)

    return value
def _normalize_advanced_template_question_text(template_question: dict) -> dict:
    """
    legacy AI 고급 template을 V2 화면 문체에 맞춘다.
    - title은 유지
    - body/choices/explanation은 존댓말 중심으로 정리
    """
    normalized = dict(template_question)

    normalized["body"] = _to_polite_sentence(normalized.get("body", ""))

    choices = normalized.get("choices", [])
    if isinstance(choices, list):
        normalized["choices"] = [
            _to_polite_sentence(choice)
            for choice in choices
        ]

    normalized["explanation"] = _to_polite_sentence(normalized.get("explanation", ""))
